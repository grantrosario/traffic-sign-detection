class Node(object):
    def __init__(self, inbound_nodes=[]):
        # Node(s) from which the Node receives values
        self.inbound_nodes = inbound_nodes
        # Node(s) to which this Node passes values
        self.outbound_nodes = []

        # For each outbound Node here, add this Node as an outbound Node to *that* Node
        for n in self.inbound_nodes:
            n.outbound_nodes.append(self)

        # A calculated value from Node
        self.value = None
...
class Input(Node):
    def __init__(self):
        # An input node has no inbound nodes, so no need to pass anything to
        # the Node instantiator (constructor)
        Node.__init__(self)
