...
with tf.Session() as sess:
    ...
    for i in range(EPOCHS):
        ...
        if(validation_accuracy > prev_val_acc):
            ...
            print("Saving model...")
            saver.save(sess, './models/german_recognize-9/model')
            print()
            continue
...
pickle.dump(euro_data, open("nets/german_recognition/recognize-9-data.p", 'wb'))
...
with tf.Session(graph = gg) as sess:
    sess.run(tf.global_variables_initializer())
    saver2 = tf.train.import_meta_graph("./models/german_recognize-9/model.meta")
    saver2.restore(sess, "./models/german_recognize-9/model")
    ...
    with open("german_recognition_results.txt", mode='a') as f:
        f.write("Recognize-9 Network Results\n")
        f.write("---\n")
