...
with open("nets/german_recognition/recognize-4-weights.p", mode='rb') as f:
...
for x in range(10):
        ...
        with tf.Session(graph=training_graph) as sess:
            ...
            for i in range(EPOCHS):
                ...
                if(validation_accuracy > prev_val_acc):
                    ...
                    print("Saving model...")
                    saver.save(sess, './models/transfer_recognize-4/model')
                    print()
                    continue
    ...
    with tf.Session(graph = gg) as sess:
        sess.run(tf.global_variables_initializer())
        saver2 = tf.train.import_meta_graph("./models/transfer_recognize-4/model.meta")
        saver2.restore(sess, "./models/transfer_recognize-4/model")
...
with open("transfer_recognition_results.txt", mode='w') as f:
    f.write("--------------------\n")
    f.write("--------------------\n")
    f.write("4 layer Results\n")
